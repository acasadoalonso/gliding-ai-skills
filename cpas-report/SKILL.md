...
description: >-
  ...
  statistics reports (flights/km/hours per class) — that is ss-report.
---
...
3. **Analysis.** `competition(type=analysis, comp=<key>)`. Expect the
   ...
   Count encounter review tags across ALL days — never generalise from a
   sample, reviewers tag only a minority of events:

   ```bash
   jq '[.data.days[].encounters[].encounter.comment.status]
       | group_by(.) | map({status: .[0], n: length})' $F
   ```
